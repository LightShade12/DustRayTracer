...
#include "Core/math/cuda_matrix.cuh"
...
__device__ Matrix3x3_d::Matrix3x3_d() = default;

__device__ Matrix3x3_d::Matrix3x3_d(float3 r0, float3 r1, float3 r2) {
	m_row[0] = make_float4(r0);
	m_row[1] = make_float4(r1);
	m_row[2] = make_float4(r2);
};

__device__ Matrix3x3_d Matrix3x3_d::transpose() const {
	Matrix3x3_d result;
...
__device__ float3 Matrix3x3_d::operator*(const float3& vec) const {
	float3 result;
