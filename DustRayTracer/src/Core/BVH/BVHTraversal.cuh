...
__device__ void traverseBVH(const Ray& ray, const int root_node_idx, HitPayload* closest_hitpayload, const SceneData* scenedata) {
	...
	while (stackPtr > 0) {
		...
		{
			child1_hitdist = (scenedata->DeviceBVHNodesBuffer[stackTopNode->left_child_or_triangle_indices_start_idx]).m_BoundingBox.intersect(ray);
			child2_hitdist = (scenedata->DeviceBVHNodesBuffer[stackTopNode->left_child_or_triangle_indices_start_idx + 1]).m_BoundingBox.intersect(ray);
...
__device__ const int traverseBVH_raytest(const Ray& ray, const int root_node_idx, const SceneData* scenedata) {
	...
	while (stackPtr > 0) {
		...
		if (stackTopNode->triangle_indices_count != 0) {
			...
			hit1 = (scenedata->DeviceBVHNodesBuffer[stackTopNode->left_child_or_triangle_indices_start_idx]).m_BoundingBox.intersect(ray);
			hit2 = (scenedata->DeviceBVHNodesBuffer[stackTopNode->left_child_or_triangle_indices_start_idx + 1]).m_BoundingBox.intersect(ray);

			if (hit1 > hit2) {
				if (hit1 >= 0) { nodeIdxStack[stackPtr++] = stackTopNode->left_child_or_triangle_indices_start_idx; }
				if (hit2 >= 0) { nodeIdxStack[stackPtr++] = stackTopNode->left_child_or_triangle_indices_start_idx + 1; }
			}
			else {
				if (hit2 >= 0) { nodeIdxStack[stackPtr++] = stackTopNode->left_child_or_triangle_indices_start_idx + 1; }
				if (hit1 >= 0) { nodeIdxStack[stackPtr++] = stackTopNode->left_child_or_triangle_indices_start_idx; }
